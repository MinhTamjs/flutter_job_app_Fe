...
class AuthHelper {
  ...
  static Future<bool> login(model) async {
    ...
    if (response.statusCode == 200) {
      ...
      print(
          "--my response is ${response.body}  status code is ${response.statusCode}");
      return false;
    }
  }
  ...
  static Future<bool> signup(String model) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        var newUrl = Uri.parse(response
            .headers['location']!); // Ensure the new location is provided
        response = await client.get(newUrl,
            headers: requestHeaders); // This typically would be a GET request
        return true;
      } else {
        return false;
      }
  ...
  static Future<bool> addSkill(String model) async {
    ...
    try {
      final response =
          await http.post(url, body: model, headers: requestHeaders);
